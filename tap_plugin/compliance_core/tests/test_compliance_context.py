...
import pytest
from tap_plugin.compliance_core.models import ComplianceContext
...
class TestComplianceContextModel:
    def test_create_via_service_layer(self, ctx):
        op = WriteOperation(
            verb="create_node",
            type_slug="compliance_core__compliance_context",
            payload={
        ...
        cc = ComplianceContext.all_objects.get(entity_id=result.results[0].entity_id)
    ...
    def test_multiple_contexts_coexist_per_regime(self, ctx):
        ...
        regimes = set(ComplianceContext.all_objects.values_list("regime", flat=True))
